...
public class LockingMain {
    ...
    public static void main(String[] args) {
        ...
        final List<Book> books = session.createQuery("from Book b").list(); // or session.createCriteria(Book.class).list();
        System.out.println("\n");
        ...
        List<Book> b = getBooks(session);
        for (Book book2 : b) {
        	session.load(Book.class, book2.getId(), new LockOptions(LockMode.PESSIMISTIC_FORCE_INCREMENT));
            System.out.println(b);
		}
